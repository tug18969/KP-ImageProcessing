...
#img = load_img('apple/1.jpg')  # this is a PIL image
#img.save('apple1.jpg')
#plt.imshow(img)
...
def splitImageSet(rootFolder, outFolder, p):
    ...
    print('Split Done!')
    return
rootFolder = 'images' #add the image directory
outFolder = 'split_images' #image output directory    
...
model.add(Dense(2)) #only two output classes
...
earlystopper = EarlyStopping(monitor='val_acc', patience=patience, 
                             verbose=1, mode='max')       
#check point
wdir = 'E:/KPImageProcessing/KP-ImageProcessing/python files' #work directory
