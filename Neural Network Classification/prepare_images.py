...
def store_raw_images(images_link, store_path, pic_num=1, img_dim=None, quiet=False, colortype = cv2.IMREAD_GRAYSCALE):
...
def find_uglies(folder, ugly):
    ...
    #get the ugly image
    #resize both images to 128x128 and compare
    try:
        ug = cv2.imread(ugly)
